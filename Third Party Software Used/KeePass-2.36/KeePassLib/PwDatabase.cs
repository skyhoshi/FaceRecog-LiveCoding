...
namespace KeePassLib
{
    ...
    public sealed class PwDatabase
    {
        ...
        public void New(IOConnectionInfo ioConnection, CompositeKey pwKey)
        {
            ...
            m_pgRootGroup = new PwGroup(true, true, UrlUtil.StripExtension(UrlUtil.GetFileName(ioConnection.Path)), PwIcon.FolderOpen);
        ...
        public void Open(IOConnectionInfo ioSource, CompositeKey pwKey, IStatusLogger slLogger)
        {
            ...
            {
                KdbxFile kdbx;
                if (ioSource.IsFileStream())
                {
                    m_pgRootGroup = new PwGroup(true, true, ioSource.StreamFileName, PwIcon.FolderOpen);
                }
                else
                {
                    m_pgRootGroup = new PwGroup(true, true, UrlUtil.StripExtension(UrlUtil.GetFileName(ioSource.Path)), PwIcon.FolderOpen);
                }
                m_pgRootGroup.IsExpanded = true;
                ...
                kdbx = new KdbxFile(this);
                kdbx.DetachBinaries = m_strDetachBins;

                if (ioSource.IsFileStream())
                {
                    kdbx.Load(ioSource.Stream, KdbxFormat.Default, slLogger);
                    ioSource.Stream.Close();
                }
                else
                {
